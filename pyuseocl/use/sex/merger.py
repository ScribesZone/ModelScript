...
usefull information resulting from the execution::

* errors like <>
* results of query evaluation commands ("??" and "?")
* results of "check" commands.

The problem is that the comments in soil files are replace
by blank lines. So it is not possible to get directly the
trace from the .soil output.
...
def merge(soilFile, traceFile, sexFileName=None):
    ...
    soil=NumberedFile(soilFile)
    trace=NumberedFile(traceFile)
    merged=SexFile(soil, trace, sexFileName)
